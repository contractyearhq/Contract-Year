...
function VerdictBadge({ verdict }) {
  const v = verdictColors[verdict] || verdictColors.fair
  return (
...
function RatingsCycler({ ratings }) {
  ...
  const v = verdictColors[current.verdict] || verdictColors.fair
      ...
      <section style={{ padding: '100px 40px 80px', borderBottom: '1px solid #2a2a2a', display: 'flex', alignItems: 'stretch', justifyContent: "space-between", gap: "32px", flexWrap: 'wrap' }}>
        <div style={{ flex: 1, minWidth: '280px' }}>
          ...
          <div style={{ display: 'flex', gap: '48px' }}>
            <div>
              <div style={{ fontSize: '42px', fontWeight: 900, lineHeight: 1 }}>{ratings.length}</div>
