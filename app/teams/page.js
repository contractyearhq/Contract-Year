...
function ScoreBadge({ score }) {
  ...
  return (
    <span style={{ background: color + '22', color: color, padding: '4px 12px', fontSize: '14px', fontWeight: 700, letterSpacing: '0.06em' }}>
      {score}
    </span>
  )
}
    ...
    <div>
      <section style={{ padding: '64px 40px 48px', borderBottom: '1px solid #2a2a2a' }}>
        ...
        <h1 style={{ fontSize: 'clamp(48px, 7vw, 96px)', fontWeight: 900, lineHeight: 0.92, textTransform: 'uppercase', marginBottom: '20px', letterSpacing: '-0.02em' }}>
          Team Contract<br /><span style={{ color: '#1D9E75' }}>Scores.</span>
        </h1>
      ...
      <section style={{ borderBottom: '1px solid #2a2a2a' }}>
        <div style={{ padding: '32px 40px 16px', fontSize: '11px', fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', color: '#555550', borderBottom: '1px solid #2a2a2a' }}>
          NFL — {nflTeams.length} Teams Rated
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr', padding: '10px 40px', borderBottom: '1px solid #1a1a1a' }}>
          {['Team', 'Overall', 'Value', 'Risk', 'Flexibility'].map(h => (
            <div key={h} style={{ fontSize: '11px', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#555550' }}>{h}</div>
          ))}
        </div>
        {nflTeams.map(function(team) {
          return (
            <Link key={team.slug} href={'/team/' + team.slug} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr', padding: '20px 40px', borderBottom: '1px solid #2a2a2a', textDecoration: 'none' }}>
              <div>
