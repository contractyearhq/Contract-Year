...
function EraBadge({ era }) {
  const map = {
    '80s': { color: '#C8A882', bg: 'rgba(200,168,130,0.15)' },
    '90s': { color: '#E8192C', bg: 'rgba(232,25,44,0.15)' },
    '2000s': { color: '#00B4FF', bg: 'rgba(0,180,255,0.15)' },
    '2010s': { color: '#FF3B30', bg: 'rgba(255,59,48,0.15)' },
  }
  const e = map[era] || { color: '#9a9894', bg: 'rgba(154,152,148,0.15)' }
  return (
    <span style={{ background: e.bg, color: e.color, padding: '3px 10px', fontSize: '12px', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase' }}>
      {era}
    </span>
  )
}
...
export default async function ThrowbacksPage({ searchParams }) {
  ...
  const filtered = throwbacks ? throwbacks.filter(t => {
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: '1px', background: '#2a2a2a', borderBottom: '1px solid #2a2a2a' }}>
        {filtered.map(function(t) {
          const stripeColor = {
            '80s': '#C8A882', '90s': '#E8192C',
            '2000s': '#00B4FF', '2010s': '#FF3B30',
          }[t.era] || '#7B68EE'
            ...
            <Link key={t.slug} href={'/throwbacks/' + t.slug} className="throwback-card" style={{ '--stripe-color': stripeColor }}>
              ...
              <div style={{ paddingLeft: '8px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  ...
                  <span style={{ fontSize: '12px', fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#7B68EE' }}>
                    Read →
                  </span>
                </div>
              </div>
