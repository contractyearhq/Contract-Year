  ...
  return (
    <div>
      <section style={{ padding: '64px 40px 48px', borderBottom: '1px solid #2a2a2a', position: 'relative' }}>
        ...
        <Link href="/teams" style={{ fontSize: '12px', fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#555550', display: 'inline-block', marginBottom: '32px', textDecoration: 'none' }}>
          Back to All Teams
        </Link>
        <div style={{ fontSize: '12px', fontWeight: 600, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#555550', marginBottom: '12px' }}>
          {team.league} · Contract Analysis
        </div>
        <h1 style={{ fontSize: 'clamp(48px, 7vw, 96px)', fontWeight: 900, lineHeight: 0.92, textTransform: 'uppercase', letterSpacing: '-0.02em', marginBottom: '32px', color: '#F5F2EC' }}>
          {team.name}
        </h1>
        <div style={{ display: 'flex', gap: '48px', flexWrap: 'wrap', alignItems: 'center' }}>
          <div style={{ textAlign: 'center' }}>
            <div style={{ width: '120px', height: '120px', borderRadius: '50%', border: '4px solid ' + scoreColor, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 8px' }}>
              <span style={{ fontSize: '42px', fontWeight: 900, color: scoreColor }}>{team.score_overall}</span>
            </div>
            <div style={{ fontSize: '11px', color: '#555550', letterSpacing: '0.08em', textTransform: 'uppercase' }}>Overall Score</div>
          </div>
          ...
          <div style={{ marginLeft: 'auto' }}>
            ...
            <div style={{ fontSize: '36px', fontWeight: 900, color: '#F5F2EC' }}>{'$' + (team.total_payroll / 1000000).toFixed(0) + 'M'}</div>
          </div>
        </div>
      </section>
      <section style={{ padding: '48px 40px', borderBottom: '1px solid #2a2a2a', maxWidth: '720px' }}>
        <div style={{ fontSize: '11px', fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', color: '#555550', marginBottom: '16px' }}>Contract Year Analysis</div>
        <p style={{ fontSize: '18px', color: '#9a9894', lineHeight: 1.75 }}>{team.summary_text}</p>
      </section>
      {team.full_analysis && (
        <section style={{ padding: '48px 40px', borderBottom: '1px solid #2a2a2a', maxWidth: '720px' }}>
          <div style={{ fontSize: '11px', fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', color: accentColor, marginBottom: '16px' }}>The Full Picture</div>
          <p style={{ fontSize: '17px', color: '#9a9894', lineHeight: 1.75 }}>{team.full_analysis}</p>
        </section>
      )}
      {(team.best_contract_player || team.worst_contract_player) && (
        <section style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', borderBottom: '1px solid #2a2a2a', gap: '1px', background: '#2a2a2a' }}>
          {team.best_contract_player && (
            <div style={{ background: '#111', padding: '40px' }}>
              <div style={{ fontSize: '11px', fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', color: '#1D9E75', marginBottom: '12px' }}>Best Contract</div>
              <div style={{ fontSize: '22px', fontWeight: 800, color: '#F5F2EC', marginBottom: '12px' }}>{team.best_contract_player}</div>
              <p style={{ fontSize: '15px', color: '#9a9894', lineHeight: 1.65 }}>{team.best_contract}</p>
            </div>
          )}
          {team.worst_contract_player && (
            <div style={{ background: '#111', padding: '40px' }}>
              <div style={{ fontSize: '11px', fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', color: '#E24B4A', marginBottom: '12px' }}>Biggest Risk</div>
              <div style={{ fontSize: '22px', fontWeight: 800, color: '#F5F2EC', marginBottom: '12px' }}>{team.worst_contract_player}</div>
              <p style={{ fontSize: '15px', color: '#9a9894', lineHeight: 1.65 }}>{team.worst_contract}</p>
            </div>
          )}
        </section>
      )}
      <section style={{ padding: '48px 40px', borderBottom: '1px solid #2a2a2a' }}>
        <div style={{ fontSize: '14px', color: '#555550', marginBottom: '16px' }}>Agree with the breakdown? Drop your verdict on Instagram.</div>
